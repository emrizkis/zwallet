...
class Resource<out T> (val state: State, val data: T?, val message: String?) {
    companion object{
        ...
        fun <T> error(data: T, message: String?): Resource<T> = Resource(State.SUCCESS, data, message)
        fun <T> loading(data: T): Resource<T> = Resource(State.SUCCESS, data, message = null)
    }
}